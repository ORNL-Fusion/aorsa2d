...
cc
cc                                 coded by alan kaylor cline
cc                           from fitpack -- january 26, 1987
cc                        a curve and surface fitting package
cc                      a product of pleasant valley software
cc                  8603 altus cove, austin, texas 78759, usa
cc
cc this subroutine returns approximations to
cc       sinhm(x) = sinh(x)/x-1
cc       coshm(x) = cosh(x)-1
cc and
cc       coshmm(x) = (cosh(x)-1-x*x/2)/(x*x)
cc with relative error less than 4.0e-14.
cc
cc on input--
cc
cc   x contains the value of the independent variable.
cc
cc   isw indicates the function desired
cc           = -1 if only sinhm is desired,
cc           =  0 if both sinhm and coshm are desired,
cc           =  1 if only coshm is desired,
cc           =  2 if only coshmm is desired,
cc           =  3 if both sinhm and coshmm are desired.
cc
cc on output--
cc
cc   sinhm contains the value of sinhm(x) if isw .le. 0 or
cc   isw .eq. 3 (sinhm is unaltered if isw .eq.1 or isw .eq.
cc   2).
cc
cc   coshm contains the value of coshm(x) if isw .eq. 0 or
cc   isw .eq. 1 and contains the value of coshmm(x) if isw
cc   .ge. 2 (coshm is unaltered if isw .eq. -1).
cc
cc and
cc
cc   x and isw are unaltered.
cc
cc-----------------------------------------------------------
cc
c      data sp14/.227581660976348e-7/,
c     *     sp13/.612189863171694e-5/,
c     *     sp12/.715314759211209e-3/,
c     *     sp11/.398088289992973e-1/,
c     *     sq12/.206382701413725e-3/,
c     *     sq11/-.611470260009508e-1/,
c     *     sq10/.599999999999986e+1/
c      data sp25/.129094158037272e-9/,
c     *     sp24/.473731823101666e-7/,
c     *     sp23/.849213455598455e-5/,
c     *     sp22/.833264803327242e-3/,
c     *     sp21/.425024142813226e-1/,
c     *     sq22/.106008515744821e-3/,
c     *     sq21/-.449855169512505e-1/,
c     *     sq20/.600000000268619e+1/
c      data sp35/.155193945864942e-9/,
c     *     sp34/.511529451668737e-7/,
c     *     sp33/.884775635776784e-5/,
c     *     sp32/.850447617691392e-3/,
c     *     sp31/.428888148791777e-1/,
c     *     sq32/.933128831061610e-4/,
c     *     sq31/-.426677570538507e-1/,
c     *     sq30/.600000145086489e+1/
c      data sp45/.188070632058331e-9/,
c     *     sp44/.545792817714192e-7/,
c     *     sp43/.920119535795222e-5/,
c     *     sp42/.866559391672985e-3/,
c     *     sp41/.432535234960858e-1/,
c     *     sq42/.824891748820670e-4/,
c     *     sq41/-.404938841672262e-1/,
c     *     sq40/.600005006283834e+1/
c      data cp5/.552200614584744e-9/,
c     *     cp4/.181666923620944e-6/,
c     *     cp3/.270540125846525e-4/,
c     *     cp2/.206270719503934e-2/,
c     *     cp1/.744437205569040e-1/,
c     *     cq2/.514609638642689e-4/,
c     *     cq1/-.177792255528382e-1/,
c     *     cq0/.200000000000000e+1/
c      data zp4/.664418805876835e-8/,
c     *     zp3/.218274535686385e-5/,
c     *     zp2/.324851059327161e-3/,
c     *     zp1/.244515150174258e-1/,
c     *     zq2/.616165782306621e-3/,
c     *     zq1/-.213163639579425e0/,
c     *     zq0/.240000000000000e+2/
cc
c      ax = abs(x)
...
cc
c      if (ax .gt. 3.9) go to 2
c      xs = ax*ax
c      if (ax .gt. 2.2) go to 1
cc
cc sinhm approximation on (0.,2.2)
cc
c      sinhm = xs*((((sp14*xs+sp13)*xs+sp12)*xs+sp11)*xs+1.)/
c     .             ((sq12*xs+sq11)*xs+sq10)
c      return
cc
cc sinhm approximation on (2.2,3.9)
cc
c    1 sinhm = xs*(((((sp25*xs+sp24)*xs+sp23)*xs+sp22)*xs+sp21)
c     .        *xs+1.)/((sq22*xs+sq21)*xs+sq20)
c      return
c    2 if (ax .gt. 5.1) go to 3
cc
cc sinhm approximation on (3.9,5.1)
cc
c      xs = ax*ax
c      sinhm = xs*(((((sp35*xs+sp34)*xs+sp33)*xs+sp32)*xs+sp31)
c     .        *xs+1.)/((sq32*xs+sq31)*xs+sq30)
c      return
c    3 if (ax .gt. 6.1) go to 4
cc
cc sinhm approximation on (5.1,6.1)
cc
c      xs = ax*ax
c      sinhm = xs*(((((sp45*xs+sp44)*xs+sp43)*xs+sp42)*xs+sp41)
c     .        *xs+1.)/((sq42*xs+sq41)*xs+sq40)
c      return
cc
cc sinhm approximation above 6.1
cc
c    4 expx = exp(ax)
c      sinhm = (expx-1./expx)/(ax+ax)-1.
c      return
cc
cc coshm and (possibly) sinhm approximation
cc
c    5 if (isw .ge. 2) go to 7
c      if (ax .gt. 2.2) go to 6
c      xs = ax*ax
c      coshm = xs*(((((cp5*xs+cp4)*xs+cp3)*xs+cp2)*xs+cp1)
c     .        *xs+1.)/((cq2*xs+cq1)*xs+cq0)
c      if (isw .eq. 0) sinhm = xs*((((sp14*xs+sp13)*xs+sp12)
c     .          *xs+sp11)*xs+1.)/((sq12*xs+sq11)*xs+sq10)
c      return
c    6 expx = exp(ax)
...
cc
c    7 xs = ax*ax
c      if (ax .gt. 2.2) go to 8
c      coshm = xs*((((zp4*xs+zp3)*xs+zp2)*xs+zp1)*xs+1.)/
c     .             ((zq2*xs+zq1)*xs+zq0)
c      if (isw .eq. 3) sinhm = xs*((((sp14*xs+sp13)*xs+sp12)
c     .          *xs+sp11)*xs+1.)/((sq12*xs+sq11)*xs+sq10)
c      return
c    8 expx = exp(ax)
