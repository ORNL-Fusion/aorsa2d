...
c
     ...
     .   n_bin, upshift, i_write, xk_cutoff, zLoc, eNormIN )
...
*----------------------------------------------------------------------
      ...
      double precision :: barrier_time

      real :: eNormIN, zLoc(nnodey)
...
!      real count(0 : 10000, 1), sum_count
     ...
     .     uzx(nxdim, nydim), uzy(nxdim, nydim), uzz(nxdim,nydim)
      ...
      allocate(b_sum(NUPER, NUPAR) )
      allocate(c_sum(NUPER, NUPAR) )
      allocate(e_sum(NUPER, NUPAR) )
      allocate(f_sum(NUPER, NUPAR) )
...
*      endif
      ...
      if(myid .eq. 0) then
*     If first core on SMP node, take all work to distribute to GPU
*     Allocate memory on the GPU
         call qlsum_gpu_initialize(nuper,
     .        nupar, nzeta_wdot,
     .        nkdim1, nkdim2, mkdim1, mkdim2,
     .        lmaxdim, nkx1, nkx2, nky1, nky2,
     .        xkxsav, xkysav,
     .        exk(nkdim1, mkdim1), eyk(nkdim1, mkdim1),
     .        ezk(nkdim1, mkdim1))
...
*     Do no work if not core 0 on an SMP node
         start = 2
         finish = 1
      endif
...
!           ----------------------------------
     ...
     .           wdot_sum, sum_fx0, sum_fy0, W, ZSPEC, 
...
!     --------------------------------
     ...
     .             2, MPI_COMM_WORLD, ierr)
      ...
      barrier_time = omp_get_wtime()
      call blacs_barrier(icontxt, 'All')
      barrier_time = omp_get_wtime() - barrier_time

      if(mod(myid, 16) .eq. 0) then
         print *, "Barrier time: ", barrier_time
      endif
...
*           --------------
      ...
      if(mycore .eq. 0) then
*     If first core on SMP node, free GPU memory
         call qlsum_gpu_cleanup()
      endif
