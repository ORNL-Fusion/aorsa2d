...
!-----------------------------------------
      ...
      real, dimension (:), allocatable :: yplot
...
!     ------------------------
      ...
      allocate (ex(nxmx, nymx), ey(nxmx, nymx), ez(nxmx, nymx), &
...
!      write(6, *)"xplotm(i) = "
!      write (6, 310) (xplotm(i),  i = 1, nxplot)

      do j = 1, nyplot
       yplot(j) = rmin + (j-1) * dyplot
       yplotm(j) = -yplot(j)
      end do
...
!       write( 6,2001)k, phi(k), curent(k), cursum(k), csum
      end do
...
!      call fftin2(cursum, work, nphi3d, nphmax, ntmin, ntmax, cn)
...
!         write(16,2001)k, phi3d(k), cur3d(k), cursum(k), csum
...
!       if(nphi .eq. 16) cnmod2(nt) = 0.0
         xnphi(nt) = real(nphi)
      end do

      read(166, 309) nnodex, nnodey
      read(166, 310) (capr(i), i = 1, nnodex)
      read(166, 310) (capz(j), j = 1, nnodey)
      read(166, 310) psilim
      read(166, 310) ((rho(i, j), i = 1, nnodex), j = 1, nnodey)

      read(166, 309) nphi_number
      read(166, 309) (nphi_array(n), n = 1, nphi_number)
...
!     -------------------------------------------------------------
      do 9000 nsum = 1, nphi_number
       ...
       nt =  nphi - nphi1 + 1
         ...
         read(166,310) ((eplus(i, j), i = 1, nnodex), j = 1, nnodey)
         read(166,310) ((eminus(i, j),  i = 1, nnodex), j = 1, nnodey)
         read(166,310) ((eb(i, j),     i = 1, nnodex), j = 1, nnodey)

       read(166,310) ((ex(i, j), i = 1, nnodex), j = 1, nnodey)
         read(166,310) ((ey(i, j),  i = 1, nnodex), j = 1, nnodey)
         read(166,310) ((ez(i, j),     i = 1, nnodex), j = 1, nnodey)

         read(166,310) ((bxwave(i, j), i = 1, nnodex), j = 1, nnodey)
         read(166,310) ((bywave(i, j), i = 1, nnodex), j = 1, nnodey)
         read(166,310) ((bzwave(i, j), i = 1, nnodex), j = 1, nnodey)
         ...
         read(34,310) ptot, pcrto2, pcito2, xjtot
...
!     -------------------------------------------
      pscale = 1.0
!DLG: changed this to > instead of == 0, not sure if it
!     was failing due to low resolution issues only.

      if(pabs_sum .gt. 0.0 )pscale = prfin / pabs_sum
...
!             ntilda(i,j,k) = - zi / (omgrf * qe) * div_j
!             ntilda_real(i,j,k) = real(ntilda(i,j,k))
...
!     .   redotj3sum, redotj4sum,
!     .   redotj5sum, redotj6sum,
!     .   xjprl_sum, wdotesum,
!     .   wdot1sum,  wdot2sum,
!     .   wdot3sum,  wdot4sum,
!     .   wdot5sum,  wdot6sum)
...
!     ------------------------------------------
 ...
 2969 format('SCALARS redotj float 1')
      ...
      write(143, 2948)
 2948 format('SCALARS ealpha_sum_mod float 1')
      write(143, 2849)
      ...
      do i = 1, nnodex
         do k = 1, nphi3d
          ...
          if (k .ge. khalf) kshift = k - khalf + 1
            ...
            freal(i, k) = real(ealpha_sum(i, jhalf, kshift))
            fimag(i, k) = imag(ealpha_sum(i, jhalf, kshift))
          power(i, k) = real(redotj(i, jhalf, kshift))
          capr_plot(i, k) = capr(i)
...
!      write(141, 2847)
 ...
 3848 format('SCALARS Re_E_alpha float 1')
      write(141, 2849)
      write (141, 3411) ((freal(i, k), i = 1, nnodex), k = 1, nphi3d)

      write(141, 3849)
 3849 format('SCALARS Im_E_alphafloat 1')
      write(141, 2849)
      write (141, 3411) ((fimag(i, k), i = 1, nnodex), k = 1, nphi3d)

      write(141, 3859)
 3859 format('SCALARS Re_EdotJ float 1')
      write(141, 2849)
      write (141, 3411) ((power(i, k), i = 1, nnodex), k = 1, nphi3d)
...
!      write(142, 2847)
 ...
 3853 format('SCALARS capr_plot float 1')
      ...
      deallocate (ex, ey, ez, bxwave, bywave, bzwave )
...
88887 format(2i10)
  899 format(1h ,5x,12h      cpu = ,1pe12.4,4h min/ &
             1h ,5x,12h       io = ,1pe12.4,4h min/ &
             1h ,5x,12h      sys = ,1pe12.4,4h min/ &
             1h ,5x,12h      mem = ,1pe12.4,4h min/ &
             1h ,5x,12h    total = ,1pe12.4,4h min)
...
!     skip a line
 ...
 1163 format(1h ,5x,6h  k   ,2x,6h phi  ,6x,6hre cur,4x,8h im cur , &
             4x,8h re jsum,4x,8h im jsum, &
             4x,8h re csum,4x,8h im csum, &
             4x,8h        ,4x,8h        )
 1164 format(1h ,5x,6hnphi  ,2x,6h  k// ,6x,6h pabs ,5x,8h weight , &
             5x,8h re pc  ,4x,8h im pc  , &
             4x,8h pabs wt,4x,8h gammacd, &
             4x,8h  damp1 ,4x,8h        )
 1165 format(1h ,5x,6h  k   ,2x,6h phi  ,6x,6hre cur,4x,8h im cur , &
             4x,8hre jthsm,4x,8him jthsm, &
             4x,8h re epsi,4x,8h im epsi, &
             4x,8h        ,4x,8h        )
  162 format(1h1)
 ...
 1010 format(3x,42h   real power transferred from antenna  = , &
         1pe12.4,9h watts   / &
            ,3x,47h   imaginary power transferred from antenna  = , &
         1pe12.4,9h watts   )
 1011 format(3x,46hreal part of antenna impedance (resistance) = , &
         1pe12.4,9h ohms    / &
            ,3x,50himaginary part of antenna impedance (reactance) = , &
         1pe12.4,9h ohms    )
