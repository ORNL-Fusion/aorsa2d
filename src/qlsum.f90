...
!DLG:   Remove xkprln = xkprl_eff
             if ( ndist .lt. 9 ) xkprln = xkprl_eff      
...
!DLG:   Remove the xkprln = xkprl_eff
             if ( ndist .lt. 9 ) xkprln = xkprl_eff      
...
!       sumwdot_31_dlg = 0.0
      ...
      facte = (nwcw - sinth**2) /  upar0
          ...
          i = int((UPAR0 - UPAR(1)) * dui) + 1
      i_uprl = i
        sum_wdot_res(i_uPrl) = sum_wdot_res(i_uPrl) + 1
          p = (UPAR0 - UPAR(i)) * dui
      !DLG
      dfduper0 = dfduper(k_uper, NUPAR)
          if (i .ne. NUPAR) then
