#define MBYTES1D(x) kind(x)*size(x)/1024d0**2
#define MBYTES2D(x) kind(x)*size(x,1)*size(x,2)/1024d0**2
...
contains
    ...
    function init_gridBlock ( nR, nZ, rMin, rMax, zMin, zMax ) result ( grid )
        ...
        Mem%nAllocations = Mem%nAllocations + 5
        Mem%MBytes = Mem%MBytes + MBYTES1D(grid%rNorm) 
        Mem%MBytes = Mem%MBytes + MBYTES1D(grid%zNorm) 
        Mem%MBytes = Mem%MBytes + MBYTES1D(grid%R) 
        Mem%MBytes = Mem%MBytes + MBYTES1D(grid%z) 
        Mem%MBytes = Mem%MBytes + MBYTES1D(grid%kPhi) 
            ...
            Mem%nAllocations = Mem%nAllocations + 4
            Mem%MBytes = Mem%MBytes + MBYTES2D(grid%xx)
            Mem%MBytes = Mem%MBytes + MBYTES2D(grid%yy) 
            Mem%MBytes = Mem%MBytes + MBYTES2D(grid%drBfn_bfn) 
            Mem%MBytes = Mem%MBytes + MBYTES2D(grid%dzBfn_bfn) 
    ...
    subroutine labelPts ( gAll, nPts_tot )
        ...
        Mem%nAllocations = Mem%nAllocations + 2
        Mem%MBytes = Mem%MBytes + MBYTES2D(bndryBlockID)
        Mem%MBytes = Mem%MBytes + MBYTES1D(bndryType)
