      ...
      integer, parameter :: n_u_dim = 200
      integer, parameter :: n_theta_dim = 160
...
!DLG:   Set n_distribution = 0 for numerical maxwellian testing
      n_distribution = 1
      xktev = 17.1e+03
      xn0 = 1.9403e+18
      amu = 1.0
...
!DLG:   Set enorm to enorm_factor instead of the way Fred had it.
      enorm_aorsa_kev = enorm_factor 
 
      enorm_aorsa_joules = 1000. * enorm_aorsa_kev * e             
      vc_mks_aorsa = sqrt(2.0 * enorm_aorsa_joules / xm)
      vc_mks_cql3d  = vc_mks_aorsa
...
!        -------------------------------------------------------------------
!DLG:   Numerical maxwellian parameters
         amu = 2.0
         xmi =  xmih * amu
...
!DLG:   [OLD] Read in the particle list derived f

    Emax = 0.5 * xmi * vc_mks**2
    Enorm = Emax / 1.6e-19
    !call dlg_particle_f ( capR, y, uPerp, uPara, nxmx, nymx, nuper, nupar, enorm, xmi )

    !do i_psi = 1, n_psi
    !    f_cql_cart(:,:,i_psi)   = p_f_vv
    !    df_cql_uprp(:,:,i_psi)  = p_dfduPerp
    !    df_cql_uprl(:,:,i_psi)  = p_dfduPar
    !end do

!DLG:   Write the numerical maxwellian into an nc file for comparison
    
    write (*,*) 'WRITING output/aorsa_num_max.nc ...'
    ncFileName = 'output/aorsa_num_max.nc'
    ...
    call check ( nf90_def_var ( nc_id, "vc_mks", NF90_REAL, &
        scalar_id, vc_mks_id ) )
    call check ( nf90_def_var ( nc_id, "xkt", NF90_REAL, &
       (/ n_psi_id /), xkt_id ) ) 
    call check ( nf90_def_var ( nc_id, "Enorm", NF90_REAL, &
       scalar_id, Enorm_id ) ) 
    call check ( nf90_def_var ( nc_id, "f_cql_cart", NF90_REAL, &
      (/ n_uper_id, n_upar_id, n_psi_dim_id /), f_cql_cart_id ) )
!    call check ( nf90_def_var ( nc_id, "p_f_rzvv", NF90_REAL, &
!      (/ n_uper_id, n_upar_id /), p_f_rzvv_id ) )
!    call check ( nf90_def_var ( nc_id, "p_dfduPer", NF90_REAL, &
!      (/ n_uper_id, n_upar_id /), p_dfduPer_id ) )
!    call check ( nf90_def_var ( nc_id, "p_dfduPar", NF90_REAL, &
!      (/ n_uper_id, n_upar_id /), p_dfduPar_id ) )
!
    call check ( nf90_def_var ( nc_id, "dfduper", NF90_REAL, &
      (/ n_uper_id, n_upar_id, n_psi_dim_id /), dfduper_id ) )
    call check ( nf90_def_var ( nc_id, "dfdupar", NF90_REAL, &
      (/ n_uper_id, n_upar_id, n_psi_dim_id /), dfdupar_id ) )
    call check ( nf90_def_var ( nc_id, "uPerp", NF90_REAL, &
      (/ n_uper_id /), uPerp_id ) )
    call check ( nf90_def_var ( nc_id, "uPara", NF90_REAL, &
      (/ n_upar_id /), uPara_id ) )

    call check ( nf90_enddef ( nc_id ) )
    call check ( nf90_put_var ( nc_id, f_cql_id, f_cql ) )
    call check ( nf90_put_var ( nc_id, u_id, u ) )
    call check ( nf90_put_var ( nc_id, theta_id, theta ) )
    !call check ( nf90_put_var ( nc_id, fnorm_id, fnorm_ ) )
    call check ( nf90_put_var ( nc_id, vc_mks_id, vc_mks ) )
    !call check ( nf90_put_var ( nc_id, xkt_id, xkt ) )
    call check ( nf90_put_var ( nc_id, Enorm_id, Enorm ) )
    call check ( nf90_put_var ( nc_id, f_cql_cart_id, f_cql_cart ) )
    call check ( nf90_put_var ( nc_id, dfduper_id, df_cql_uprp ) )
    call check ( nf90_put_var ( nc_id, dfdupar_id, df_cql_uprl ) )
    call check ( nf90_put_var ( nc_id, uPerp_id, uperp ) )
    call check ( nf90_put_var ( nc_id, uPara_id, upara ) )
!    call check ( nf90_put_var ( nc_id, p_f_rzvv_id, p_f_rzvv ) )
!    call check ( nf90_put_var ( nc_id, p_dfduPer_id, p_dfduPerp ) )
!    call check ( nf90_put_var ( nc_id, p_dfduPar_id, p_dfduPar ) )
! 
...
!      write (238, 310) (((df_cql_uprp(i_uperp, i_upara, i_psi), &
!     &  i_uperp = 1, nuper), i_upara = 1, nupar), i_psi = 1, n_psi)

!      write (238, 310) (((df_cql_uprl(i_uperp, i_upara, i_psi), &
!     &  i_uperp = 1, nuper), i_upara = 1, nupar), i_psi = 1, n_psi)

!      write (238, 310) ((DFDUPER(i_uperp, i_upara), &
!     &  i_uperp = 1, nuper), i_upara = 1, nupar)

!      write (238, 310) ((DFDUPAR(i_uperp, i_upara), &
!     &  i_uperp = 1, nuper), i_upara = 1, nupar)
...
!     Write Point.3D file
!     ------------------------
!      write(139, *) '  v_para    v_perp     log_f     rho'
!      do i_uperp = 1, nuper
!         do i_upara = 1, nupar
!       do i_psi = 1, n_psi
!          xlogf = -100.0
!          if(f_cql_cart(i_uperp, i_upara, i_psi) .gt. 0.0) then
!             xlogf = log10(f_cql_cart(i_uperp, i_upara, i_psi))
!          end if
!          rho10 = rho_a(i_psi) * 5.
!               write(139, 3410) upara(i_upara), uperp(i_uperp),  &
!     &           xlogf, rho10
!            end do
!    end do
!      end do
...
!     ----------------------------------------
!     Write Point.vtk file "structured points"
!     ----------------------------------------
!      number_points = nupar * nuper * n_psi
!      dupara = upara(2) - upara(1)
!      duperp = uperp(2) - uperp(1)
!      drho = rho_a(2) - rho_a(1)
!      drho10 = 10. * drho
...
!      write(140, 2842) upara(1), uperp(1), rho_a(1)
! 2842 format('ORIGIN', 3f9.3)
 
!      write(140, 2843) dupara, duperp, drho10
! 2843 format('SPACING', 3f9.3)      
      
!      write(140, 2844) number_points
! 2844 format('POINT_DATA', i10)      
      
!      write(140, 2848)
! 2848 format('SCALARS volume_scalars float 1')       
